using PsjLib.Base;
using PsjLib.Base.Capabilities;
using PsjLib.NVFamily.Capabilities;
using PsjLib.Transport;

namespace PsjLib.NVFamily.NV403CLE;

/// <summary>
/// Device implementation for NV40/3CLE closed-loop three-channel amplifier.
/// </summary>
public sealed class NV403CLEDevice : NVFamilyDevice
{
    static NV403CLEDevice()
    {
        DeviceModelRegistry.Registry["NV40/3CLE"] = static (transport, id) => new NV403CLEDevice(transport, id);
    }

    /// <summary>
    /// Initializes a new NV40/3CLE device wrapper.
    /// </summary>
    /// <param name="transportType">Transport backend.</param>
    /// <param name="identifier">Transport identifier.</param>
    public NV403CLEDevice(TransportType transportType, string identifier)
        : base(transportType, identifier)
    {
        Knob = new NVCLEKnob(CapabilityWriteAsync, new Dictionary<string, string>
        {
            [NVKnob.CmdMode] = "encmode",
            [NVKnob.CmdSampleTime] = "enctime",
            [NVKnob.CmdStepLimit] = "enclim",
            [NVKnob.CmdAccelExponent] = "encexp",
            [NVKnob.CmdStepOpenLoop] = "encstol",
            [NVCLEKnob.CmdStepClosedLoop] = "encstcl",
        });

        MultiSetpoint = new MultiSetpoint(CapabilityWriteAsync, new Dictionary<string, string>
        {
            [MultiSetpoint.CmdSetpoints] = "setall",
        }, MaxChannelCount);

        MultiPosition = new MultiPosition(CapabilityWriteAsync, new Dictionary<string, string>
        {
            [MultiPosition.CmdPositions] = "measure",
        });
    }

    /// <inheritdoc/>
    public override string? DeviceId => "NV40/3CLE";
    /// <inheritdoc/>
    internal override string NVFamilyIdentifier => "NV403CLE";
    /// <inheritdoc/>
    public override int MaxChannelCount => 3;
    /// <inheritdoc/>
    internal override NVFamilyChannel CreateChannel(int channelId) => new NV403CLEChannel(channelId, WriteChannelAsync);

    /// <summary>
    /// Gets typed NV40/3CLE channels keyed by channel identifier.
    /// </summary>
    public new IReadOnlyDictionary<int, NV403CLEChannel> Channels => ChannelsInternal.ToDictionary(kv => kv.Key, kv => (NV403CLEChannel)kv.Value);
    /// <summary>
    /// Front-panel knob configuration capability including closed-loop step size.
    /// </summary>
    public NVCLEKnob Knob { get; }
    /// <summary>
    /// Synchronous setpoint write capability for all channels.
    /// </summary>
    /// <remarks>
    /// To use this capability, all 3 channels must have an actuator connected and modulation source must be set to "serial". 
    /// Otherwise, the amplifier will ignore the command.
    /// </remarks>
    public MultiSetpoint MultiSetpoint { get; }
    /// <summary>
    /// Synchronous position readback capability for all channels.
    /// </summary>
    public MultiPosition MultiPosition { get; }
}
